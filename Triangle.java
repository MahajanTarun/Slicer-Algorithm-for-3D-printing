...
public class Triangle {
	...
	*/
	...
	/**
	* Substracts a vector s from the triangles corners.
	* @param s vector which will be substracted
	* @return new Triangle
	*/
	...
	*/
	...
	public Vector combination(Plane p, int i, int j) {
		...
		if (distanceI * distanceJ < 0) {
			...
			return corner[i].add(edge.mull(factor));
		} else if (distanceI == 0) {
			return corner[i];
		} else if (distanceJ == 0) {
			return corner[j];
	...
	*/
	...
	public String toString() {
		return "Triangle_[corner = " + Arrays.toString(corner) + ",_normal = " + normal + "]";
	}
}